    ...
    <div class="task-manager">
        ...
        <div class="top-controls glass-panel">
            <div class="view-toggle">
                <button @click="viewMode = 'list'" class="btn-toggle" :class="{ active: viewMode === 'list' }"
                    title="Lista">
                    <font-awesome-icon icon="list" />
                </button>
                <button @click="viewMode = 'grid'" class="btn-toggle" :class="{ active: viewMode === 'grid' }"
                    title="Cuadrícula">
                    <font-awesome-icon icon="grip" />
                </button>
                <button @click="viewMode = 'calendar'" class="btn-toggle" :class="{ active: viewMode === 'calendar' }"
                    title="Calendario">
        ...
        <section class="task-form card hover-lift">
            ...
            <form @submit.prevent="handleSubmit">
                <div class="form-group">
                    <input v-model="form.title" type="text" placeholder="¿Qué necesitas hacer hoy?" required
                        class="input-modern" />
                </div>

                <div class="form-group">
                    <textarea v-model="form.description" placeholder="Añade detalles..." rows="2"
                    ...
                    <div class="form-group half">
                        ...
                        <div class="select-wrapper">
                            <select v-model="form.parent_id" class="input-modern">
                                ...
                                <option v-for="task in potentialParents" :key="task.id" :value="task.id">
                                    {{ task.title }}
                                </option>
                            </select>
                        </div>
                ...
                <div class="form-group">
                    <input v-model="form.due_date" type="datetime-local" class="input-modern" />
                </div>

                <div class="form-group">
                    <label class="label-sub">Etiquetas</label>
                    <div class="tags-input">
                        <span v-for="tag in tagStore.tags" :key="tag.id" class="tag-chip"
                            :class="{ selected: form.tags.includes(tag.id) }" :style="{ '--tag-color': tag.color }"
                ...
                <div class="form-actions">
                    ...
                    <button v-if="editingTask" type="button" @click="cancelEdit" class="btn btn-text">
        ...
        <transition name="fade" mode="out-in">
            <TaskCalendar v-if="viewMode === 'calendar'" :tasks="store.filteredTasks" @edit-task="startEdit" />
            ...
            <section v-else class="task-view-container">
                ...
                <div v-else class="tasks-wrapper" :class="viewMode">
                    <draggable v-model="localTasks" item-key="id" class="drag-area" :class="viewMode" @end="onDragEnd"
                        handle=".drag-handle">
                        <template #item="{ element: task }">
                            <div class="task-card hover-lift" :class="{
                                ...
                                <div class="task-body">
                                    <div class="task-top">
                                        ...
                                        <h4 class="task-title">
                                            <font-awesome-icon v-if="isBlocked(task)" icon="lock" class="blocked-icon"
                                                title="Bloqueada por dependencias" />
                                            {{ task.title }}
                                        </h4>
                                    ...
                                    <div class="task-meta">
                                        ...
                                        <span v-if="task.due_date" class="meta-badge date"
                                    ...
                                    <div v-if="task.dependencies && task.dependencies.length > 0" class="deps-list">
                                        ...
                                        <span v-for="depId in task.dependencies" :key="depId" class="dep-tag"
                                            :class="{ done: isTaskDone(depId) }">
                                            {{ getTaskTitle(depId) }}
                                        </span>
                                    </div>

                                    <div class="tags-list" v-if="task.tags && task.tags.length">
                                        <span v-for="tag in task.tags" :key="tag.id" class="mini-tag"
                                            :style="{ color: tag.color }">#{{ tag.name }}</span>
                                    </div>
                                </div>

                                <div class="task-actions-overlay">
                                    <button @click.stop="store.updateTask(task.id, { completed: !task.completed })"
                                        class="action-btn check" :disabled="isBlocked(task) && !task.completed"
                                    ...
                                    <button @click.stop="confirmDelete(task.id)" class="action-btn delete"
                                        title="Eliminar">
...
/* Ensure selects inside TaskManager have room for the custom arrow */
select.input-modern {
...
.dep-tag {
    ...
    /* Default assumes blocked/incomplete logic style? No wait. */
